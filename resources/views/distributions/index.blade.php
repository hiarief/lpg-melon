...
$rankPiutang = $customers->map(function ($c) use ($customerTotals) {
    ...
    $pctLunas = $t['total_value'] > 0 ? round($t['paid'] / $t['total_value'] * 100) : 100;
    return [
        'id'        => $c->id,
        'name'      => $c->name,
        'type'      => $c->type,
        'tagihan'   => $t['total_value'],
        'bayar'     => $t['paid'],
        'piutang'   => $t['total_value'] - $t['paid'],
        'qty'       => $t['qty'],
        'pct_lunas' => $pctLunas,
    ];
})->filter(fn($c) => $c['piutang'] > 0)->sortByDesc('piutang')->values();

$custBarNames = $customers
    ->filter(fn($c) => ($customerTotals[$c->id]['qty'] ?? 0) > 0)
    ->map(fn($c) => ['name' => $c->name, 'qty' => $customerTotals[$c->id]['qty'], 'type' => $c->type])
...
$konsentrasi = $s['allQty'] > 0 ? round($custBarNames->max('qty') / $s['allQty'] * 100) : 0;
    ...
    <div style="padding:12px;">
            ...
            <div class="scroll-x">
                <table class="rank-table">
                    ...
                    <tbody>
                        ...
                        @php
                            $pctShare = $s['piutang'] > 0 ? round($rc['piutang'] / $s['piutang'] * 100) : 0;
                        ...
                        <tr>
                            ...
                            <td>
                                ...
                                <div class="progress-bar">
                                    <div class="progress-fill" style="width:{{ $rc['pct_lunas'] }}%;background:{{ $barColor }};"></div>
                                </div>
                            </td>
        ...
        <div class="col">
            <div class="card" style="padding:12px;margin-top:10px">
                ...
                @php
                $indikators = [
                    ['Tingkat pelunasan',        number_format($s['rasioLunas'],1).'%',  'dari total tagihan',                                              $s['rasioLunas'],       $s['rasioLunas']>=90?'var(--melon)':($s['rasioLunas']>=70?'#f59e0b':'#dc2626')],
                    ['Hari aktif',               $s['activeDays'].' hari',               $s['rasioAktif'].'% hari kerja',                                   $s['rasioAktif'],       'var(--melon)'],
                    ['Variasi harga',            $s['variasiHarga'].'%',                 'Rp '.number_format($s['hargaMin']).' – '.number_format($s['hargaMax']), min($s['variasiHarga']*5,100), $s['variasiHarga']<5?'var(--melon)':'#f59e0b'],
            ...
            <div class="card" style="padding:12px;margin-top:10px">
                ...
                <div class="pill-box">
                    ...
                    <div class="pill pill-red">
                        <span class="pill-icon">!</span>
                        <span>Pelunasan {{ number_format($s['rasioLunas'],1) }}% — prioritaskan penagihan ke <strong>{{ $rankPiutang->first()['name'] ?? '-' }}</strong></span>
                    </div>
                    @endif
                    @if($konsentrasi > 40)
                    ...
                    <div class="pill pill-orange">
                        <span class="pill-icon">⚠</span>
                        <span>Harga/tab bervariasi {{ $s['variasiHarga'] }}% — standarisasi perlu ditinjau</span>
                    </div>
                    @endif
                    @if($s['rasioAktif'] < 80)
                    <div class="pill pill-blue">
                        <span class="pill-icon">i</span>
                        <span>Distribusi aktif {{ $s['rasioAktif'] }}% hari kerja — potensi peningkatan frekuensi</span>
                    </div>
                    @endif
                    @if($s['rasioLunas'] >= 90 && $konsentrasi <= 40 && $s['variasiHarga'] <= 5 && $s['rasioAktif'] >= 80)
    ...
    <div style="padding:12px;">
        ...
        <div class="dist-kpi-grid">
            ...
            <div class="kpi-card">
                ...
                <span class="kpi-value" style="color:#7c3aed;">{{ number_format($pr['projTren']) }} tab</span>
            ...
            <div class="kpi-card">
                ...
                <span class="kpi-value" style="color:#d97706;">{{ $pr['sisaKalender'] }} hari</span>
                <span class="kpi-sub">~{{ $pr['estHariSisa'] }} hari aktif est.</span>
            </div>
        </div>
        ...
        <div style="display:grid;grid-template-columns:repeat(3,1fr);gap:8px;margin-bottom:14px;">
            @foreach([
                ['Pesimis',       $pr['projMin'],  '#dc2626', 'jika distribusi melambat'],
                ['Tren saat ini', $pr['projTren'], '#1d4ed8', 'berdasarkan rata-rata aktual'],
