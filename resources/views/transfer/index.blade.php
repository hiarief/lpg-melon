...
{{--
    Semua kalkulasi berat (tren harian, komposisi kurir, indikator, mutasi saldo)
    telah dipindahkan ke TransferController. View ini hanya bertanggung jawab
    pada presentasi data.
--}}
...
.tf-tab-strip::-webkit-scrollbar { display: none; }
...
<div x-data="{ tabActive: 'setoran' }">
            ...
            <form method="GET" action="{{ route('transfer.index') }}">
                <select name="period_id" onchange="this.form.submit()" class="field-select" style="padding:6px 10px;font-size:12px;width:auto">
                    @foreach($periods as $p)
                        <option value="{{ $p->id }}" @selected($p->id == $period->id)>{{ $p->label }}</option>
                    @endforeach
                </select>
            </form>
        </div>
    ...
    <div style="display:grid;grid-template-columns:1fr 1fr;gap:8px;margin-bottom:10px">
        @foreach([
            ['Saldo Awal Penampung',        'Rp '.number_format($period->opening_penampung), '#1d4ed8'],
            ['Total Masuk (Setoran Kurir)',  'Rp '.number_format($totalDeposited),            'var(--melon-dark)'],
            ['Total Keluar (ke Rek Utama)',  'Rp '.number_format($totalTransferred),           '#dc2626'],
            ['Total Biaya Admin',            'Rp '.number_format($totalAdmin),                 '#dc2626'],
        ...
        <div class="card" style="grid-column:span 2;padding:10px 12px;border-left:3px">
            ...
            <div style="font-size:10px;color:var(--text3)">
                = {{ number_format($period->opening_penampung) }}
                + {{ number_format($totalDeposited) }}
                − {{ number_format($totalTransferred) }}
            </div>
        </div>
    </div>
    ...
    <div class="tf-flow-strip">
        ...
        <div class="tf-flow-node" style="background:#eff6ff;border-left:2px solid #1d4ed8;">
            ...
            <div class="tf-flow-val"   style="color:{{ $penampungNow >= 0 ? '#1d4ed8' : '#dc2626' }};">
                {{ number_format(round($penampungNow / 1000)) }}k
            </div>
        </div>
    </div>

    {{-- ══ TAB STRIP ══ --}}
    <div class="tf-tab-strip">
        @foreach([
            ['setoran',  '<path d="M8 11V4m0 7-3-3m3 3 3-3"/><rect x="2" y="13" width="12" height="1.5" rx=".75"/>',  'Setoran'],
            ['transfer', '<path d="M8 5v7M8 5l-3 3M8 5l3 3"/><rect x="2" y="2" width="12" height="1.5" rx=".75"/>',   'Transfer'],
            ['analisis', '<path d="M2 13l3-4 3 2 3-5 3 4"/>',                                                          'Analisis'],
            ['riwayat',  '<circle cx="8" cy="8" r="6"/><path d="M8 5v3.5l2 2"/>',                                      'Riwayat'],
        ] as [$tab, $svgPath, $label])
        <button class="tf-tab-btn" :class="tabActive==='{{ $tab }}'?'active':''" @click="tabActive='{{ $tab }}'">
            <svg viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.5">{!! $svgPath !!}</svg>
            {{ $label }}
        </button>
        @endforeach
    </div>
                ...
                <table class="mob-table">
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td class="r" style="color:{{ $dep->admin_fee > 0 ? '#dc2626' : 'var(--text3)' }}">
                                {{ $dep->admin_fee > 0 ? 'Rp ' . number_format($dep->admin_fee) : '—' }}
                            </td>
                            <td class="r bold" style="color:var(--melon-dark)">Rp {{ number_format($dep->amount) }}</td>
                            <td style="color:var(--text3)">{{ $dep->reference_no ?: '—' }}</td>
                            @if($period->status === 'open')
                            <td>
                                <form method="POST" action="{{ route('transfer.deposit.destroy', $dep) }}" style="display:inline"
                ...
                <table class="mob-table">
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td>
                                @forelse($tf->deliveryOrders as $do)
                                    <span class="badge badge-blue" style="margin-right:3px;margin-bottom:2px">
                                        {{ $do->outlet->name }} Rp{{ number_format($do->pivot->amount_allocated / 1000) }}k
                                    </span>
                                @empty
                            ...
                            @if($period->status === 'open')
                            <td>
                                <form method="POST" action="{{ route('transfer.account.destroy', $tf) }}" style="display:inline"
                                      onsubmit="return confirm('Hapus? Status DO akan di-reset.')">
                                    @csrf @method('DELETE')
                                    <button type="submit" class="link-btn" style="color:#dc2626">Hapus</button>
                                </form>
                            </td>
                        ...
                        @empty
                        <tr>
                            <td colspan="6" style="text-align:center;padding:20px;color:var(--text3)">Belum ada transfer.</td>
                        </tr>
                        @endforelse
                    </tbody>
                    <tfoot>
        ...
        <div class="s-card">
            ...
            <div style="padding:12px 14px;">
                ...
                <div style="position:relative;width:100%;height:220px;">
                    <canvas id="tfTrendChart" role="img" aria-label="Chart tren setoran masuk dan transfer keluar penampung per hari">
                        Tren setoran dan transfer harian.
                    </canvas>
                </div>
            </div>
        </div>
            ...
            <div class="s-card" style="margin-bottom:0;">
                ...
                <div style="padding:12px 14px;">
                    <div style="position:relative;height:140px;">
                        <canvas id="tfDonutChart" role="img" aria-label="Donut chart setoran per kurir">Setoran per kurir.</canvas>
                    </div>
                ...
                <div style="padding:12px 14px;">
                    ...
                    <div class="ind-row">
                        <div class="ind-meta">
                            ...
                            <div class="ind-track">
                                <div class="ind-fill" style="width:{{ $bar }}%;background:{{ $col }};"></div>
                            </div>
                        </div>
                        <div class="ind-right">
        ...
        <div class="s-card">
            ...
            <div class="scroll-x">
                <table class="mob-table">
                    <thead>
                        <tr>
                            <th>Tanggal</th><th>Keterangan</th>
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td>{{ $row['desc'] }}</td>
                            <td class="r" style="color:{{ $row['type']==='in' ? 'var(--melon-dark)' : 'var(--text3)' }};font-weight:{{ $row['type']==='in' ? 600 : 400 }}">
                                {{ $row['type'] === 'in' ? 'Rp ' . number_format($row['amount']) : '—' }}
                            </td>
                            <td class="r" style="color:{{ $row['type']==='out' ? '#dc2626' : 'var(--text3)' }};font-weight:{{ $row['type']==='out' ? 600 : 400 }}">
                                {{ $row['type'] === 'out' ? 'Rp ' . number_format($row['amount']) : '—' }}
                            </td>
                            <td class="r bold" style="color:{{ $row['balance'] >= 0 ? '#1d4ed8' : '#dc2626' }}">
...
{{-- ══ DATA JSON UNTUK CHART (disuntikkan controller via $chartData) ══ --}}
<div id="tfChartData"
     data-labels='@json($chartData["trendLabels"])'
     data-dep='@json($chartData["trendDep"])'
     data-tf='@json($chartData["trendTf"])'
     data-saldo='@json($chartData["trendSaldo"])'
     data-kurir-names='@json($chartData["kurirNames"])'
     data-kurir-totals='@json($chartData["kurirTotals"])'
...
(function () {
    ...
    const fmtK = v => {
        const a = Math.abs(v);
        return (v < 0 ? '-' : '') + (a >= 1_000_000
            ? (Math.round(a / 100_000) / 10).toFixed(1) + 'jt'
            : Math.round(a / 1_000) + 'k');
    };
    ...
    if (trendEl && labels.length) {
        new Chart(trendEl, {
            data: {
                ...
                datasets: [
                    {
                        type: 'bar', label: 'Setoran masuk', data: depData.map(v => v / 1000),
                        backgroundColor: 'rgba(67,160,71,0.4)', borderColor: '#43A047',
                        borderWidth: 1, borderRadius: 3, stack: 'a', order: 2,
                    },
                    {
                        type: 'bar', label: 'Transfer keluar', data: tfData.map(v => v / 1000),
                        backgroundColor: 'rgba(59,130,246,0.4)', borderColor: '#3b82f6',
                        borderWidth: 1, borderRadius: 3, stack: 'b', order: 2,
                    },
                    {
                        type: 'line', label: 'Saldo penampung', data: saldoData.map(v => v / 1000),
                        borderColor: '#b45309', borderWidth: 2, borderDash: [5, 3],
                        pointRadius: 3, pointBackgroundColor: '#b45309',
                        tension: 0.35, fill: false, yAxisID: 'y2', order: 1,
    ...
    if (donutEl && kurirNames.length) {
        ...
        kurirNames.forEach((name, i) => {
            ...
            row.innerHTML = `<span style="width:9px;height:9px;border-radius:2px;background:${colors[i]};flex-shrink:0;"></span>`
                          + `<span style="flex:1;color:var(--text2);">${name}</span>`
                          + `<span style="font-weight:600;color:var(--text1);">${pct}%</span>`;
            leg.appendChild(row);
        });
    }
})();
